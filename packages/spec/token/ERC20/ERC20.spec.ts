...
export interface ERC20SpecArgs {
  supply: BigNumber;
  mint: (address: string, amount: BigNumber) => Promise<ContractTransaction>;
  burn: (address: string, amount: BigNumber) => Promise<ContractTransaction>;
}

export function describeSpecOfERC20(
  deploy: () => Promise<IERC20>,
  { supply, mint, burn }: ERC20SpecArgs,
  skips?: string[],
) {
  const describe = specFilter(skips);
